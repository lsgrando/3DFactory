...
ensureColumn(conn, 'itens', 'responsavel_id', 'INTEGER');
ensureColumn(conn, 'movimentacoes_financeiras', 'comprovante', 'TEXT');
    ...
    const r = conn
      .prepare(
        `INSERT INTO movimentacoes_financeiras
          (tipo, valor, conta_id, forma_pagamento, categoria, categoria_detalhe, descricao, pedido_id, responsavel_id, reembolsado, reembolso_id, estornado, estorno_de, data, criado_em, comprovante)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .run(
        m.tipo,
