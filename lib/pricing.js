...
//   custoFilamento = soma, por filamento usado na peça, de (precoPorKg / 1000) * pesoGramas
...
function calcularPrecificacao({ filamentos, tempoHoras, potenciaW, tarifaKwh, margemPercentual, margemFixa }) {
  const custoFilamento = (filamentos || []).reduce(
    (s, f) => s + (Number(f.precoPorKg) / 1000) * Number(f.pesoGramas || 0),
    0
  );
